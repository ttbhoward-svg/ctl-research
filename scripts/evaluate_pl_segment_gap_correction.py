#!/usr/bin/env python3
"""Evaluate offline PL segment-based gap correction (with optional drift correction)."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Offline PL segment gap correction evaluator")
    ...
    seg_bias = estimate_segment_gap_bias(base.l2.detail_df, SEGMENTS)
    manifest_seg = apply_segment_gap_bias(manifest, seg_bias)

    seg_only = _diag(can_df, ts_adj, ts_unadj, manifest_seg, cfg.tick_size, cfg.max_day_delta)
    ...
    combined = _diag(can_regime, ts_adj, ts_unadj, manifest_seg, cfg.tick_size, cfg.max_day_delta)

    payload = {
        "symbol": "PL",
        "segment_gap_biases": [b.to_dict() for b in seg_bias],
        "regime_offsets": [o.to_dict() for o in offsets],
        "baseline": _snap(base),
        "segment_gap_only": _snap(seg_only),
        "combined": _snap(combined),
        "delta_segment_gap_vs_baseline": {
            "mean_gap_diff": seg_only.l3.mean_gap_diff - base.l3.mean_gap_diff,
            "mean_drift": seg_only.l4.mean_drift - base.l4.mean_drift,
        },
    ...
    print("PL Segment Gap Correction (Offline)")
    print("Segment biases (median signed canonical_gap - ts_gap):")
    for b in seg_bias:
        print(f"- {b.label}: {b.start}..{b.end} n={b.n_rows} median={b.median_signed_gap_delta:.4f}")

    for name in ["baseline", "segment_gap_only", "combined"]:
        s = payload[name]
        print(f"\n{name}:")
        print(f"  decision={s['decision']} strict/policy={s['strict']}/{s['policy']} gap={s['mean_gap_diff']:.4f} drift={s['mean_drift']:.4f}")

    d1 = payload["delta_segment_gap_vs_baseline"]
    d2 = payload["delta_combined_vs_baseline"]
    print("\nDelta segment_gap_only - baseline:")
